...
function sumNumbers(array) {
  const updatedArray = _.compact(array)
  return _.sum(updatedArray)
}
...
function countClassesByInstructor(collection, instructor) {
  const updatedArr = _.filter(collection, { "instructor": instructor }).length
  return updatedArr > 0 ? updatedArr : "There is no instructor by that name."
}
...
function removeInactiveMembers(collection) {
  return _.remove(collection, { "currentMember": true });
}
