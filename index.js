...
function groupClassByInstructor(collection) {
  const groupClass = _.groupBy(collection, "instructor");
  return groupClass;
};
...
function omitAgeFromMembers(collection) {
  ...
  for (const member of collection) {
    const removedAge = _.omit(member, ['age'] )
    newArray.push(removedAge);
  }
  return newArray;
};
...
function countClassesByInstructor(collection, instructorName) {
  let classArray = [];
  for (const classObj of collection) {
    console.log(classObj)
    let objValues = _.values(classObj)
    if (objValues.includes(instructorName)){
...
function removeInactiveMembers(collection) {
 const activeMembers = _.filter(collection, 'currentMember');
...
function getUniqueClasses(collection) {
  const uniqueTitle =  _.uniqBy(collection, 'title');
  return _.map(uniqueTitle, ({ title, priceInCents }) => ({
    title, priceInCents,
  }));
}
...
function orderClassesByTitleAndLevel(collection) {
  const order = _.orderBy(collection, ['title', 'level'] ['asc' , 'desc']);
  return _.map(order, ({title, instructor, level}) => ({title, instructor, level})
  )
}
