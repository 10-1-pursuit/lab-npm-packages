...
const removeAges = require("./data/members-without-ages.json");
...
function sumNumbers(array) {
  return _.compact(array).reduce((a, b) => a + b);
}
...
function omitAgeFromMembers(collection) {
  return _.map(collection, (member) => _.omit(member, "age")); //adding member to iterate collection and omit age in iterator if found
}
/**
...
function countClassesByInstructor(collection, instructorName) {
  ...
  if (myfilteredCollection > 0) {
    ...
    return "There is no instructor by that name."; // if the length is 0 or negative then that means there are no instructors inside the filteredCollection
  }
}
...
function orderClassesByTitleAndLevel(collection) {
  return _.map(
    _.orderBy(collection, ['title', 'level'] ['asc' , 'desc']), ({title, instructor, level}) => ({title, instructor, level})
  )
}
