...
function sumNumbers(array) {
  const filteredArray = _.remove(array)
...
function groupClassByInstructor(collection) {
  const instructors = _.groupBy(collection, "instructor")
  return instructors
}
...
function omitAgeFromMembers(collection) { 
  const newArray =  _.remove(collection, "age")
  return newArray
}
...
function countClassesByInstructor(collection, instructor) {
  for(const nameOfInstructor of collection){
    if(nameOfInstructor.instructor === instructor){
      const instructors = _.pick(collection, ["instructor"])
      return _.keys(instructors).length
    }
...
function getUniqueClasses(collection) {
  return collection.map((uniqueClassObj) =>
  _.pick(uniqueClassObj, ["title", "priceInCents"])
  );
}
...
function orderClassesByTitleAndLevel(collection) {
  const classesOrderedByTitleAndLevel = _.orderBy(collection, ["title", "level"], ["asc", "desc"])
  return classesOrderedByTitleAndLevel.map((classTitleAndLevelObj) =>
    _.pick(classTitleAndLevelObj, ["title", "level", "instructor"])
  );
