...
function sumNumbers(array) {
  const falsyRemoved = _.compact(array);
  return _.sum(falsyRemoved);
}
...
function countClassesByInstructor(collection, instructor) {
  const ClassByInstructor = groupClassByInstructor(collection)
  return _.size(ClassByInstructor[instructor]) || "There is no instructor by that name."
}
...
function getUniqueClasses(collection) {
  ...
  return _.map(uniqueClasses, ({ title, priceInCents }) => ({ title, priceInCents }));
}
...
function orderClassesByTitleAndLevel(collection) {
  return _.orderBy(collection, ['title', 'level'], ['asc', 'desc']).map(({ title, instructor, level }) => ({
    title,
    instructor,
    level
  }));
